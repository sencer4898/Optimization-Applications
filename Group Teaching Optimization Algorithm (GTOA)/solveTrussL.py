...
def solveTrussL(struct):
    ...
    for case in cases:
        ...
        reactions("-dynamic")
        for i in supports:
            r.append(nodeReaction(i[0]))
        u=[] # node displacements
        """for i in nodes: # node displacements (given order)
            u.append(nodeDisp(i[0]))"""
        for i in range(1,len(nodes)+1): # node displacements (sorted)
            u.append(nodeDisp(i))
        n=[] # axial forces
        s=[] # member stresses
        """for i in members: # member forces and stresses (given order)
            force=flt(eleResponse(i[0],"axialForce")) # member force
            n.append(force)
            s.append(force/sections[i[1]-1]) # member stress"""
        for i in range(1,len(members)+1): # member forces and stresses (sorted)
            force=flt(eleResponse(i,"axialForce")) # member force
            n.append(force)
            mbrSection=next(j[1] for j in members if j[0]==i)
            s.append(force/sections[mbrSection-1]) # member stress
        # ---------------------------------------------------------------------------
